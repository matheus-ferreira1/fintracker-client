...
export const dashboardService = {
  async getDashboard(userId: string, filters: DashboardFilters): Promise<DashboardResponse> {
    const { currentMonth, currentYear, previousMonth, previousYear } = this.parsePeriod(
      filters.period
    )
    ...
    const [
      ...
      dashboardRepository.getPeriodSummary(userId, currentStartDate, currentEndDate),
      dashboardRepository.getPeriodSummary(userId, previousStartDate, previousEndDate),
      dashboardRepository.getMonthlyAggregations(userId, 6),
      dashboardRepository.getExpensesByCategory(userId, currentStartDate, currentEndDate),
      dashboardRepository.getRecurringTransactionsSummary(
        userId,
        currentStartDate,
        currentEndDate
      )
    ...
    const balance = this.calculateBalanceMetric(currentPeriodSummary, previousPeriodSummary)
    ...
    const nextMonthPrediction = this.calculateNextMonthPrediction(recurringTransactionsSummary)
  ...
  calculatePercentageComparison(current: number, previous: number): PercentageComparison {
  ...
  calculateBalanceMetric(
    ...
    const previousBalance = previousPeriod.totalIncome - previousPeriod.totalExpenses

    return {
      balance: currentBalance,
      comparison: this.calculatePercentageComparison(currentBalance, previousBalance)
    }
  },

  calculateIncomeMetric(currentIncome: number, previousIncome: number): IncomeMetric {
    return {
      totalIncome: currentIncome,
      comparison: this.calculatePercentageComparison(currentIncome, previousIncome)
    }
  },

  calculateExpenseMetric(currentExpenses: number, previousExpenses: number): ExpenseMetric {
    return {
      totalExpenses: currentExpenses,
      comparison: this.calculatePercentageComparison(currentExpenses, previousExpenses)
    }
  },
  ...
  formatExpensesByCategory(
    ...
    const formattedCategories = categories.map(cat => ({
      ...
      percentage:
        totalExpenses > 0 ? Number(((cat.totalAmount / totalExpenses) * 100).toFixed(2)) : 0
    }))
  ...
  calculateNextMonthPrediction(summary: {
    totalIncome: number
    totalExpenses: number
    incomeCount: number
